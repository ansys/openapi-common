...
class ApiClientFactory:
    ...
    Handles set-up of retry strategy, session-level timeout and any additional requests configuration.
    Authentication must be configured afterwards using one of the other class methods.
    ...
    def with_anonymous(self: Api_Client_Factory) -> Api_Client_Factory:
        """Sets up client authentication for anonymous use. This does not configure any authentication or
    ...
    def with_credentials(
        ...
        """Sets up client authentication for use with provided credentials.
    ...
    def with_oidc(
        ...
        """Sets up client authentication for use with OpenID Connect.
    ...
    def __handle_initial_response(
        ...
        """Verifies that an initial 401 is returned if we expect to require authentication. If a 2XX response
    ...
    def __get_authenticate_header(
        ...
        """Helper method. Extracts the ``WWW-Authenticate`` header from a requests response.
...
class OIDCSessionBuilder:
    """Helps create OpenID Connect sessions from different types of input, and provides OIDC-specific configuration
    options.
    ...
    def with_token(
        ...
        """Uses a provided access token or refresh token to authenticate the session.

        If an access token is provided then it will be used immediately. When it expires the token will
    ...
    def authorize(self, login_timeout: int = 60) -> ApiClientFactory:
        """Authenticates the user interactively by opening a web browser and waiting for the user to log in.
...
class _RequestsTimeoutAdapter(HTTPAdapter):
    ...
    def send(
        ...
        verify : Union[bool, str]
            Either a boolean which controls whether we verify the server's TLS certificate, or a string
            which must be a path to a CA bundle to use.
